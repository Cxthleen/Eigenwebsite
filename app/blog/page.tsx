...
export default function Blog() {
  ...
  return (
    <main className="max-w-3xl mx-auto px-6 py-14">
      ...
      <div className="bg-white dark:bg-dark-card border-2 border-pink-border dark:border-dark-border rounded-2xl px-5 py-4 mb-8 flex items-center gap-3 w-fit">
        <div>
          <p className="text-xs font-semibold text-lavender dark:text-dark-text-soft uppercase tracking-wide">
            Total logged
          </p>
          <p key={totalHours} className="heading-font text-xl font-bold text-plum dark:text-dark-text animate-pop">
            {totalHours} hours
          </p>
        </div>
      </div>
  ...
  <p className="text-xs text-plum-soft dark:text-dark-text-soft mt-3">
    {totalHours} / {GOAL_HOURS} hours
      ...
      <form
        ...
        className="bg-white dark:bg-dark-card border-2 border-pink-border dark:border-dark-border rounded-3xl p-6 shadow-sm mb-10 flex flex-col gap-4"
      >
      ...
      <div className="flex flex-col gap-4">
        {loading && <p className="text-plum-soft dark:text-dark-text-soft text-sm">Loading entries...</p>}

        {!loading && entries.length === 0 && (
          <p className="text-plum-soft dark:text-dark-text-soft text-sm">No entries yet — add your first day above!</p>
        )}

        {entries.map((entry) => (
          <div
            key={entry.id}
            className="bg-white dark:bg-dark-card border-2 border-pink-border dark:border-dark-border rounded-3xl p-5 shadow-sm relative animate-card-in"
          >
            <button
              onClick={() => handleDelete(entry.id)}
              className="absolute top-4 right-4 text-xs text-lavender dark:text-dark-text-soft hover:text-pink-hover"
              aria-label="Delete entry"
            >
              ✕
            </button>
            <div className="flex items-center gap-3 mb-2">
              <span className="heading-font font-bold text-plum dark:text-dark-text">{entry.date}</span>
              <span className="text-xs px-2 py-1 bg-pink-tag dark:bg-dark-bg rounded-full text-pink-tag-text dark:text-dark-text font-semibold">
                {entry.hours} hrs
              </span>
            </div>
            <p className="text-sm text-plum-soft dark:text-dark-text-soft whitespace-pre-wrap">{entry.note}</p>
          </div>
        ))}
      </div>
    </main>
